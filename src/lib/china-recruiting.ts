...
function validDate(value: string | null | undefined): boolean {
  return value == null || /^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(Date.parse(`${value}T00:00:00`));
}

export function deriveDeadlineUrgency(deadline: string | null, today: string): DeadlineUrgency {
  if (!deadline) return "Not published";
  const days = Math.ceil((Date.parse(`${deadline}T00:00:00`) - Date.parse(`${today}T00:00:00`)) / 86_400_000);
...
export function recommendationScore(record: ChinaCampusOpportunity, today: string): number {
  ...
  return record.fitScore
    + ({ P1: 30, P2: 15, P3: 5 })[record.priority]
    + (urgency === "Closing in 7 days" ? 25 : urgency === "Closing in 14 days" ? 15 : 0)
...
export function chinaPipelineMetrics(records: ChinaCampusOpportunity[], today: string) {
  ...
  const count = (status: ChinaRecruitingStatus) => active.filter((record) => record.status === status).length;
  return {
    toApply: count("To Apply"), applied: count("Applied"), oa: count("OA"),
    interview: count("Interview"), offer: count("Offer"), rejected: count("Rejected"),
    closingIn7Days: active.filter((record) => deriveDeadlineUrgency(record.deadline, today) === "Closing in 7 days").length,
  };
}

function validateImport(value: unknown, index: number): value is ChinaOpportunityImportInput {
  if (typeof value !== "object" || value === null) return false;
  const item = value as Record<string, unknown>;
  return typeof item.company === "string" && Boolean(item.company.trim())
    && typeof item.position === "string" && Boolean(item.position.trim())
    ...
    && typeof item.hiringSeason === "string" && Boolean(item.hiringSeason.trim())
    && typeof item.officialApplyLink === "string" && validHttps(item.officialApplyLink)
    && typeof item.sourceName === "string" && Boolean(item.sourceName.trim())
    && typeof item.sourceUrl === "string" && validHttps(item.sourceUrl)
    ...
    && typeof item.fitScore === "number" && item.fitScore >= 0 && item.fitScore <= 100
    && Number.isFinite(index);
}

function duplicateKey(item: Pick<ChinaCampusOpportunity, "company" | "position" | "officialApplyLink">): string {
...
export function importChinaOpportunityJson(
  ...
  let parsed: unknown;
  try { parsed = JSON.parse(raw); } catch { return { records: existing, inserted: 0, updated: 0, skipped: 0, errors: ["Invalid JSON."] }; }
  const inputs = Array.isArray(parsed) ? parsed : [parsed];
  const errors: string[] = [];
  const valid = inputs.filter((item, index) => {
    const ok = validateImport(item, index);
    if (!ok) errors.push(`Record ${index + 1} is missing required fields or contains invalid values.`);
    return ok;
  }) as ChinaOpportunityImportInput[];
  const timestamp = options.now ?? new Date().toISOString();
  const today = timestamp.slice(0, 10);
  const records = structuredClone(existing);
  let inserted = 0; let updated = 0; let skipped = errors.length;
  const importedKeys = new Set<string>();
  for (const input of valid) {
    const key = duplicateKey(input);
    if (importedKeys.has(key)) { skipped += 1; continue; }
    importedKeys.add(key);
    const existingIndex = records.findIndex((record) => duplicateKey(record) === key);
    if (existingIndex >= 0) {
