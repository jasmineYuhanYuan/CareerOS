...
export default function ErrorPage({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div role="alert" className="rounded-[1.35rem] border border-[var(--border)] bg-[var(--surface)] px-6 py-12 text-center">
      ...
      <button type="button" onClick={reset} className="mt-6 min-h-11 rounded-xl bg-[var(--text-primary)] px-5 text-sm font-medium text-[var(--surface)] hover:bg-[var(--accent)]">Try again</button>
    </div>
  );
}
