...
export async function POST(request: Request) {
  ...
  const stream = new ReadableStream({ async start(controller) {
    ...
    try {
      ...
      try { const extracted = await extractDocument(buffer, file.type); send({ stage: "extracting" }); extractedText = extracted.text; parsedData = extracted.parsedData; lastParsedAt = new Date().toISOString(); } catch { parseStatus = "failed"; parsedStatus = "error"; parseError = "The file was uploaded, but its content could not be parsed."; }
    ...
    } catch (error) { send({ stage: "error", error: error instanceof Error ? error.message : "Document upload failed." }); } finally { controller.close(); }
  }});
