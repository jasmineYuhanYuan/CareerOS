...
export async function POST(request: Request) {
  ...
  const stream = new ReadableStream({ async start(controller) {
    ...
    try {
      ...
      try { const extracted = await extractDocument(buffer, file.type); send({ stage: "extracting" }); extractedText = extracted.text; parsedData = extracted.parsedData; lastParsedAt = new Date().toISOString(); } catch (error) { console.error("Document parsing failed.", error); parseStatus = "failed"; parsedStatus = "error"; parseError = "The file was uploaded, but its content could not be parsed."; }
