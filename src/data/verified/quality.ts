...
export function validateSourceMetadata(record: SourceMetadata, id: string): string[] {
  const errors: string[] = [];
  ...
  if (!validIsoDate(record.lastVerified)) errors.push(`${id}: invalid lastVerified`);
  ...
  if (!["Verified", "Partially verified", "Archived", "Unknown"].includes(record.verificationStatus)) errors.push(`${id}: invalid verificationStatus`);
  return errors;
}
...
export function validateRegistrationPathway(record: ProfessionalRegistrationPathway): string[] {
  const errors = validateSourceMetadata(record, record.id);
