...
const jobOpportunities: Opportunity[] = jobs.map((job) => ({
  ...
  startDate:
    verifiedCareerOpportunities.find((record) => record.id === job.id)
...
export function validateOpportunities(
  records: Opportunity[],
  organisationIds = new Set(organisations.map((item) => item.id)),
): OpportunityValidationResult {
  const errors: string[] = [];
  const ids = new Set<string>();
  for (const record of records) {
    if (!record.id || !record.title || !record.organisationId)
      errors.push(`Missing required fields: ${record.id || "unknown"}`);
    if (ids.has(record.id))
      errors.push(`Duplicate opportunity ID: ${record.id}`);
    ids.add(record.id);
    if (!opportunityCategories.includes(record.category))
      errors.push(`Invalid category: ${record.id}`);
    if (!verificationStatuses.includes(record.verificationStatus))
      errors.push(`Invalid verification status: ${record.id}`);
    if (!organisationIds.has(record.organisationId))
      errors.push(`Unknown organisation: ${record.id}`);
    ...
    if (
      record.verificationStatus === "Official source" &&
      (!record.sourceUrl || !record.lastVerifiedAt)
    ) {
      errors.push(
        `Official source requires URL and last verified date: ${record.id}`,
      );
    }
