...
export function RecruitmentCalendar() {
  ...
  const records = useMemo(
    () =>
      eligibleOpportunities
        ...
        .filter(
          (item) => country === "All" || item.opportunity.country === country,
        )
        .filter((item) => lifecycle === "All" || item.lifecycle === lifecycle)
        .sort((a, b) =>
          (a.opportunity.deadline ?? "9999").localeCompare(
  ...
  const sessions = activeWorkspace.applications
    .flatMap((application) =>
  ...
  return (
    <div className="page-enter">
      <PageHeading
        eyebrow={zh ? "已核验招聘时间" : "Verified recruitment timing"}
        title={zh ? "招聘日历" : "Recruitment calendar"}
        description={
          zh
            ? "只显示官方来源中的日期。没有发布日期或截止日期的项目保持“日期未知”。"
            : "Only dates published by a verified source are shown. Records without published timing remain explicitly undated."
        }
      />
      <div className="surface-card mb-7 grid gap-4 p-4 sm:grid-cols-2">
        ...
        <label className="text-sm font-medium">
          ...
          <Select
            value={lifecycle}
            onChange={(event) =>
              setLifecycle(event.target.value as OpportunityLifecycle | "All")
            }
          >
            <option value="All">{displayUiValue("All", language)}</option>
            {(
              ...
              ] as OpportunityLifecycle[]
            ).map((value) => (
              <option key={value}>{displayUiValue(value, language)}</option>
            ))}
          </Select>
        </label>
      </div>
      <div className="grid gap-6 xl:grid-cols-[1fr_22rem]">
        <section>
          <h2 className="mb-4 font-display text-xl font-medium">
            {zh ? "已发布日期" : "Published dates"}
          </h2>
          {dated.length ? (
            <div className="space-y-3">
              {dated.map(({ opportunity, lifecycle: status }) => (
                <article
                  key={opportunity.id}
                  className="surface-card grid gap-3 p-5 sm:grid-cols-[8rem_1fr]"
                >
                  <time dateTime={opportunity.deadline} className="font-medium">
                    {formatDate(opportunity.deadline!, language)}
                  </time>
                  <div>
            ...
            <p className="surface-card border-dashed p-8 text-sm text-[var(--text-secondary)]">
              {zh
                ? "当前筛选没有已发布日期。"
                : "No published dates match these filters."}
            </p>
          )}
        </section>
        <aside className="space-y-6">
          <section className="surface-card p-5">
            <h2 className="font-display text-lg font-medium">
              {zh ? "日期未知" : "Date not published"}
            </h2>
            <ul className="mt-3 space-y-3">
              {undated.map(({ opportunity, lifecycle: status }) => (
                <li key={opportunity.id}>
                  <StatusBadge>{displayUiValue(status, language)}</StatusBadge>
          ...
          <section className="surface-card p-5">
            <h2 className="font-display text-lg font-medium">
              {zh ? "个人面试与 OA" : "Personal interview and OA sessions"}
            </h2>
            {sessions.length ? (
              <ul className="mt-3 space-y-3">
                {sessions.map(({ application, session }) => (
                  <li key={session.id}>
                    ...
                    <p className="font-medium">
                      {session.type}: {application.organisationName}
                    </p>
                    <p className="text-sm text-[var(--text-secondary)]">
              ...
              <p className="mt-3 text-sm text-[var(--text-secondary)]">
                {zh ? "没有已安排会话。" : "No scheduled sessions."}
              </p>
            )}
          </section>
        </aside>
