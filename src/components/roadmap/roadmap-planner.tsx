  ...
  return (
    <div className="page-enter">
      ...
      {Object.entries(grouped).map(([month, items]) => <section key={month} className="mb-10"><h2 className="eyebrow mb-4">{month}</h2><div className="relative ml-3 border-l border-[var(--border)] pl-7">{items?.map((item) => <article key={item.id} className={`relative border-b border-[var(--border)] py-5 first:pt-0 last:border-b-0 ${item.status === "Completed" ? "opacity-60" : ""}`}>
        <span aria-hidden="true" className={`absolute -left-[2.12rem] top-6 size-3 rounded-full border-2 border-[var(--background)] ${item.status === "Completed" ? "bg-[var(--success)]" : item.priority === "High" ? "bg-[var(--accent)]" : "bg-[var(--border-strong)]"}`} />
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
          <time dateTime={item.targetDate} className="w-24 shrink-0 text-sm font-medium text-[var(--accent)]">{item.targetDate ? new Date(`${item.targetDate}T00:00:00`).toLocaleDateString("en-AU", { day: "numeric", month: "short" }) : "No date"}</time>
          ...
          <div className="flex gap-1"><Button size="sm" variant="ghost" onClick={() => upsertRoadmapItem({ ...item, status: item.status === "Completed" ? "In progress" : "Completed" })}>{item.status === "Completed" ? "Reopen" : "Complete"}</Button><Button size="sm" variant="ghost" onClick={() => setDraft(structuredClone(item))}>Edit</Button><Button size="sm" variant="ghost" onClick={() => window.confirm(`Delete “${item.title}”?`) && deleteRoadmapItem(item.id)}>Delete</Button></div>
        </div>
      </article>)}</div></section>)}
      {Object.keys(grouped).length === 0 && <p className="surface-card border-dashed p-10 text-center text-sm text-[var(--text-secondary)]">No roadmap items match these filters.</p>}
