  ...
  return (
    <div className="page-enter">
              ...
              return (
                <article key={program.id} className="interactive-lift surface-card grid gap-5 p-5 sm:grid-cols-[1fr_auto] sm:items-center sm:p-6">
                  <div className="min-w-0">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2"><StatusBadge status="active">{calculateProgramMatch(program, activeWorkspace.profile)}% planning alignment</StatusBadge>{program.sampleData ? <Badge>{t("common.sampleNotice")}</Badge> : <StatusBadge status="positive">University source</StatusBadge>}<span className="text-[0.75rem] text-[var(--text-tertiary)]">{program.deadline ? `Published deadline: ${program.deadline}` : "Deadline not verified"}</span></div>
                  </div>
      ...
      <Dialog open={selected !== null} title={selected?.programName ?? "Program"} description={selected?.university} onClose={() => setSelected(null)}>
        {selected && <div className="space-y-5 text-sm">{selected.sampleData ? <StatusBadge status="active">Sample—verify all details</StatusBadge> : <StatusBadge status="positive">Verified university source</StatusBadge>}<p className="rounded-xl bg-[var(--surface-subtle)] p-3 text-xs text-[var(--text-secondary)]">Planning alignment compares stored interests and location only. It does not measure admission probability. Unknown requirements remain unknown.</p><section><h3 className="font-medium">Entry requirements</h3><ul className="mt-2 list-disc pl-5 text-[var(--text-secondary)]">{selected.entryRequirements.map((item) => <li key={item}>{item}</li>)}</ul></section><dl className="grid gap-4 sm:grid-cols-2"><div><dt className="font-medium">Language requirements</dt><dd className="mt-1 text-[var(--text-secondary)]">{selected.languageRequirements}</dd></div><div><dt className="font-medium">GRE</dt><dd className="mt-1 text-[var(--text-secondary)]">{selected.greRequirement}</dd></div><div><dt className="font-medium">Recommendations</dt><dd className="mt-1 text-[var(--text-secondary)]">{selected.recommendationLetters}</dd></div><div><dt className="font-medium">Tuition</dt><dd className="mt-1 text-[var(--text-secondary)]">{selected.tuitionText}</dd></div></dl><a href={selected.applicationUrl} target="_blank" rel="noreferrer" className="font-medium text-[var(--accent)]">Open official programme source ↗</a></div>}
      </Dialog>
