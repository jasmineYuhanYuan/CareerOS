# CareerOS Development Plan

## Phase 0 — Product setup

- [x] Create GitHub repository
- [x] Define product vision
- [x] Define two initial user profiles
- [x] Add repository documentation
- [x] Confirm technical stack
- [ ] Create project board
- [ ] Create initial issues

## Phase 1 — Foundation

- [x] Create Next.js TypeScript project
- [x] Configure Tailwind CSS
- [x] Add shared layout and navigation
- [ ] Configure environment variables
- [ ] Set up Supabase project
- [ ] Define database tables
- [x] Add local seed data

## Phase 2 — Core MVP

- [x] Dashboard
- [x] Career profile page
- [x] Jobs page
- [x] Companies page
- [x] Application tracker
- [x] Postgraduate planner
- [x] Career roadmap
- [x] Versioned browser persistence and reset
- [x] Local export and validated import
- [x] Responsive desktop and mobile workflows

## Phase 3 — Personalisation

- [x] Deterministic job-profile matching rules
- [ ] Skill-gap analysis
- [x] Profile-aware role recommendations
- [x] Postgraduate program comparison
- [x] Profile-aware dashboard actions
- [ ] Refine skill-gap rules with user feedback
- [ ] Add richer application activity reporting
- [ ] Validate local MVP with both test users
- [x] Add curated cross-category opportunity architecture
- [x] Add opportunity provenance validation
- [x] Add profile-specific contacts and document records
- [x] Add version 2 to version 3 local-data migration
- [ ] Complete bilingual copy coverage across legacy feature modules

## Phase 4 — AI

- [ ] JD analysis
- [ ] CV tailoring suggestions
- [ ] Cover letter drafting
- [ ] Interview question generation
- [ ] Career adviser chat

## Initial GitHub issues

1. Initialise Next.js application
2. Add navigation shell
3. Define Supabase schema
4. Add seed career profiles
5. Build dashboard cards
6. Build jobs table and filters
7. Build application Kanban
8. Build postgraduate program tracker
9. Build roadmap timeline
10. Add match-score prototype
